...
with tf.device("/gpu:1"):
    for i, (train_fold, test_fold) in enumerate(kf.split(train_,train_df['label'].values)):
        ...
        #    continue
        print(test_fold)
        #np.save('./test_fold'+str(i)+'npy',test_fold)
        '''
        print("第%s的结果"%i)
        X_train, X_valid = tf.concat([train_[train_fold, :],new_],axis=0), train_[test_fold, :]
        y_train, y_valid = np.append(train_label[train_fold],new_data_y,axis=0), train_label[test_fold]
        ...
        # 检查点保存至的目录
        checkpoint_dir = './TextBIGRU_400_cv_advfinetune_checkpoints/cv_'+str(i)+'/'
        # 检查点的文件名
        checkpoint_prefix = os.path.join(checkpoint_dir, "ckpt_{epoch}")
        model = build_model(2400, embedding_matrix)
        ...
        if i<10:
            model.load_weights(tf.train.latest_checkpoint('./TextBIGRU_400_cv_advfinetune_checkpoints/cv_'+str(i)+'/'))
        else:
            model.load_weights(tf.train.latest_checkpoint('./TextBIGRU_400_cv_finetune_checkpoints/cv_'+str(i)+'/'))
        #else:
        #    model.load_weights(tf.train.latest_checkpoint('./TextBIGRU_400_cv_checkpoints/cv_'+str(i)+'/'))
            model.fit(train_ds,epochs=2,validation_data=val_ds,callbacks=[early_stopping,plateau,checkpoint],
                verbose=1)
            model.load_weights(tf.train.latest_checkpoint('./TextBIGRU_400_cv_advfinetune_checkpoints/cv_'+str(i)+'/'))
    ...
    #GPU 释放
        ...
        #    break
        '''
#np.save("TextBIGRU_400advfientune_train_result.npy",train_pre_matrix)
#np.save("TextBIGRU_400advfinetune_valid_result.npy",valid_pre_matrix)
#np.save("TextBIGRU_400advfinetune_test_result.npy",test_pre_matrix)
